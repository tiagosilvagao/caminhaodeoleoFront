(function() {
  ...
  this.cronapi.maps.init = function( /** @type {ObjectType.OBJECT} @description {{mapId}} @blockType ids_from_screen*/ id, /** @type {ObjectType.STRING} @description {{typeMap}} @blockType util_dropdown @keys roadmap|hybrid|satellite|terrain  @values {{roadmap}}|{{hybrid}}|{{satellite}}|{{terrain}}  */ type, /** @type {ObjectType.OBJECT} @description {{centerPoint}} */ centerPoint, /** @type {ObjectType.STRING} @description {{zoom}} @blockType util_dropdown @keys 16|18|10  @values {{street}}|{{buildings}}|{{city}}  */ zoom, /** @type {ObjectType.STATEMENTSENDER} @description {{statement}} */ statement ) {
   $.getScript('https://maps.googleapis.com/maps/api/js?key='+$('#'+id).attr('token')+'&libraries=places,drawing', function() {
      ...
      map.started = true;
      // google.maps.event.addDomListener(map,'drag',function(event){
      //   debugger;
      // })
  ...
  this.cronapi.maps.createLatLngPoint = function(  /** @type {ObjectType.STRING} @description {{latitude}} */ latitude, /** @type {ObjectType.STRING} @description {{lontitude}} */ lontitude) {
      ...
      // if(window.google){
      // return new google.maps.LatLng( Number.parseFloat(latitude), Number.parseFloat(lontitude) );
      // }else
   ...
   * @name {{createWayPoint}}
   * @nameTags map|latitude|longitude|create|border|bounds|google
   * @description {{createWayPointDescription}}
   * @returns {ObjectType.OBJECT}
   */
  this.cronapi.maps.createWayPoint = function(   /** @type {ObjectType.STRING} @description {{point}} */ point) {
      return {
        "location": point
      }
  }
   ...
   * @returns {ObjectType.OBJECT}
   */
  this.cronapi.maps.createRequestDirection = function( /** @type {ObjectType.OBJECT} @description {{originPoint}} */  originPoint, /** @type {ObjectType.OBJECT} @description {{destinationPoint}} */  destinationPoint, /** @type {ObjectType.OBJECT} @description {{travelMode}} @blockType util_dropdown @keys DRIVING|TRANSIT|WALKING|BICYCLING  @values {{DRIVING}}|{{TRANSIT}}|{{WALKING}}|{{BICYCLING}}  */  travelMode, /** @type {ObjectType.OBJECT} @description {{waypoints}} */  waypoints, /** @type {ObjectType.STRING} @description {{optimizeWaypoints}} @blockType util_dropdown @keys true|false  @values {{true}}|{{false}}  */  optimizeWaypoints, /** @type {ObjectType.OBJECT} @description {{requestAdvancedOptions}} */  requestAdvancedOptions  ) {
     var request = {
       'origin': originPoint,
   ...
   * @name {{drawingManager}}
   * @nameTags map|autoComplete|direction|google|completar|property|propriedade
   * @description drawingManager
   */
  this.cronapi.maps.drawingManager = function( /** @type {ObjectType.OBJECT} @description {{mapId}} @blockType ids_from_screen*/ id, /** @type {ObjectType.STRING} @description {{drawingModes}} @blockType util_dropdown @keys polygon|circle|rectangle|marker|polyline  @values polygon|circle|rectangle|marker|polyline  */  drawingModes,  /** @type {ObjectType.STATEMENTSENDER} @description {{onRectangleDraw}} */ onRectangleDraw, /** @type {ObjectType.STATEMENTSENDER} @description {{onCircleDraw}} */ onCircleDraw , /** @type {ObjectType.STATEMENTSENDER} @description {{onMarkerDraw}} */ onMarkerDraw , /** @type {ObjectType.STATEMENTSENDER} @description {{onPolylineDraw}} */ onPolylineDraw , /** @type {ObjectType.STATEMENTSENDER} @description {{onPolygonDraw}} */ onPolygonDraw ) {
   var map = document.getElementById(id)._map;
    ...
    switch(drawingModes){
      case 'polygon':{
        draw.setOptions({ 'drawingControlOptions': {'drawingModes': ['polygon'] }});
        google.maps.event.addListener(draw, 'polygoncomplete', onPolygonDraw); 
        break;
      }
      case 'circle':{
        draw.setOptions({ 'drawingControlOptions': {'drawingModes': ['circle'] }});
        google.maps.event.addListener(draw, 'circlecomplete', onCircleDraw); 
        break;
      }
      case 'rectangle':{
        draw.setOptions({ 'drawingControlOptions': {'drawingModes': ['rectangle'] }});
        google.maps.event.addListener(draw, 'rectanglecomplete', onRectangleDraw); 
        break;
      }
      case 'marker':{
        draw.setOptions({ 'drawingControlOptions': {'drawingModes': ['marker'] }});
        google.maps.event.addListener(draw, 'markercomplete', onMarkerDraw); 
        break;
      }
      case 'polyline':{
        draw.setOptions({ 'drawingControlOptions': {'drawingModes': ['polyline'] }});
        google.maps.event.addListener(draw, 'polylinecomplete', onPolylineDraw); 
        break;
      }
    }
    draw.setMap(map);
      ...
      /**
   * @type function
   * @name {{getPolygonPaths}}
   * @nameTags map|autoComplete|direction|google|completar|property|propriedade
   * @description drawRouteDescription
   * @returns {ObjectType.OBJECT} 
   */
  this.cronapi.maps.getPolygonPaths = function( /** @type {ObjectType.OBJECT} @description {{polygon}}*/ polygon) {
   var array = [];
   $.each(polygon.getPaths().getArray()[0].getArray() , function( index, value ) {
     array.push({ 'lat': value.lat(), 'lng': value.lng()});
   });
   return array;
  }
  ...
  /**
   * @type function
   * @name {{getCircleProperties}}
   * @nameTags map|autoComplete|direction|google|completar|property|propriedade
   * @description drawRouteDescription
   * @returns {ObjectType.OBJECT} 
   */
  this.cronapi.maps.getCircleProperties = function( /** @type {ObjectType.OBJECT} @description {{circle}}*/ circle ,  /** @type {ObjectType.STRING} @description {{drawingModes}} @blockType util_dropdown @keys center|radius|bounds  @values center|radius|bounds */  property) {
   ...
   switch(property){
    ...
    /**
   * @type function
   * @name {{getRectangleProperties}}
   * @nameTags map|autoComplete|direction|google|completar|property|propriedade
   * @description drawRouteDescription
   * @returns {ObjectType.OBJECT} 
   */
  this.cronapi.maps.getRectangleProperties = function( /** @type {ObjectType.OBJECT} @description {{rectangle}}*/ rectangle ,  /** @type {ObjectType.STRING} @description {{drawingModes}} @blockType util_dropdown @keys bounds  @values bounds */  property) {
  ...
  /**
   * @type function
   * @name {{isPointOnBounds}}
   * @nameTags map|autoComplete|direction|google|completar|property|propriedade
   * @description drawRouteDescription
   * @returns {ObjectType.BOOLEAN} 
   */
  this.cronapi.maps.isPointOnBounds = function( /** @type {ObjectType.OBJECT} @description {{point}}*/ point ,  /** @type {ObjectType.OBJECT} @description {{bounds}}*/ bounds ) {
   var json = JSON.parse(bounds);
   var sw = {'lat': json.south , 'lng': json.west};
   var ne = {'lat': json.north ,'lng': json.east }
   ...
   return bound.contains(point);
    ...
    /**
   * @type function
   * @name {{getMarkerProperties}}
   * @nameTags map|autoComplete|direction|google|completar|property|propriedade
   * @description getMarkerProperties
   * @returns {ObjectType.OBJECT} 
   */
  this.cronapi.maps.getMarkerProperties = function( /** @type {ObjectType.OBJECT} @description {{marker}}*/ marker ,  /** @type {ObjectType.STRING} @description {{drawingModes}} @blockType util_dropdown @keys position  @values position */  property ) {
     switch(property){
      case 'position':{
        return {
